...
import com.milaboratory.statplots.clustering.*
import com.milaboratory.statplots.common.PlotWrapper
import com.milaboratory.statplots.heatmap.Position.*
...
class Heatmap(
    ...
    init {
        ...
        val data = _data
        var xclust: TreeNode<Any?>? = null

        this.xax = when (xOrder) {
            null -> data[x].distinct().toList().filterNotNull()

            is WithComparator -> data.rows()
                .asSequence()
                .distinctBy { it[x] }
                .sortedWith(xOrder.comparator)
                .mapNotNull { it[x] }
                .toList()
            ...
            is Hierarchical -> {
                ...
                for (row in data) {
                    xmap
                        ...
                        .computeIfAbsent(row[y]) { toDouble(row[z], xOrder.alt) }
                }
                ...
                val forClustering = xmap.mapValues { yz ->
                    yvals.map { yz.value[it] ?: xOrder.alt }.toDoubleArray()
                }.toList()

                xclust = HierarchicalClustering.clusterize(
